...
if (!sock.authState.creds.registered) {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (cmd === 'ship') {
      ...
      const percent = Math.floor(Math.random() * 101)
      const shipMsg = percent >= 80 ? '🔥 Soulmates! Perfect match!' : percent >= 60 ? '💕 Great chemistry!' : percent >= 40 ? '🙂 Could work with effort!' : percent >= 20 ? '😬 It\'s complicated...' : '💔 Not meant to be!'
      await sock.sendMessage(from, { text: `💕 *Ship Rate*\n\n*${name1}* + *${name2}*\n\n${'█'.repeat(Math.floor(percent / 10))}${'░'.repeat(10 - Math.floor(percent / 10))} ${percent}%\n\n${shipMsg}` })
    }

    if (cmd === 'roast') {
      const target = args.slice(1).join(' ') || 'you'
      const roasts = ["is the reason they put instructions on shampoo bottles! 😂", "could be used as a human alarm clock — just annoying enough to wake everyone up! 😴", "has the personality of a wet sock. 🧦", "is proof that even mistakes can walk and talk! 🚶", "is like a cloud — when they disappear, it's a beautiful day! ☁️", "reminds me of a software update — nobody wants you but you keep showing up! 💻", "is so boring, their shadow falls asleep! 😴"]
    ...
    if (cmd === 'warn') {
      ...
      warnings[num]++
      await sock.sendMessage(from, { text: `⚠️ *Warning ${warnings[num]}/3*\n\n@${num} has been warned!\n📋 *Reason:* ${reason}\n\n${warnings[num] >= 3 ? '🚨 Final warning! Next action will be a kick!' : `⚠️ ${3 - warnings[num]} warning(s) remaining!`}`, mentions: [mentioned] })
    }
